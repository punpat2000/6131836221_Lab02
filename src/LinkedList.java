...
public class LinkedList {
	...
	public void add(Object c) throws Exception {
		if (c == null || !(c instanceof Contact))
			return;
		Contact c1 = (Contact) c;
		Iterator itr = new MyListIterator(this.header);
		if (!itr.hasNext()) {
			this.insert(c1, (MyListIterator) itr);
			return;
		}

		while (itr.hasNext()) {
			Contact temp1 = (Contact) itr.next();
			String temp2 = temp1.name;
			if (c1.name.compareTo(temp2) <= 0) {
				this.insert(c1, this.findPrevious(temp1));
				return;
			}
		}
		this.insert(c1, (MyListIterator) itr);
